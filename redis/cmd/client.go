...
type BitcaskClient struct {
	...
	db *kvproject_redis.RedisDataStructure
}
...
func execClientCommand(conn redcon.Conn, cmd redcon.Command) {
	...
	case "quit":
		_ = conn.Close()
	case "ping":
		conn.WriteString("PONG") // Examine if the server is connected
	default:
		res, err := cmdFunc(client, cmd.Args[1:]) // The first args is used to know the command, it is useless now
		if err != nil {
...
func set(cli *BitcaskClient, args [][]byte) (interface{}, error) {
	...
	key, value := args[0], args[1] // args here doesn't have the first arg which is used to know the command
